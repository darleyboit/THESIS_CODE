...
#LOGGING PATENTS PER CAPITA
data$log_patents_per_capita <- log(data$patents_per_capita)
View(data)
##logged histogram -- would obv need to readjust numbers a lot 
patents_percap_distribution <- ggplot(data, aes(x = log_patents_per_capita)) +
  geom_histogram(binwidth = 0.005, fill = "red", color = "black") +
  labs(title = "Histogram of Patents Per Capita", x = "Patents Per Capita", y = "Frequency") + 
  theme(  panel.background = element_rect(fill = "white"),  # Panel background
          plot.background = element_rect(fill = "white"),   # Overall plot background
...
patentss <- ggplot(data, aes(x = patent)) +
  ...
  labs(title = "Histogram of Patents, Quantity", x = "Patent Quantity", y = "Frequency") + 
  theme(  panel.background = element_rect(fill = "white"),  # Panel background
          plot.background = element_rect(fill = "white"),   # Overall plot background
          panel.grid.major = element_line(color = "light gray"),      # Major grid lines
          panel.grid.minor = element_line(color = "white")       # Minor grid lines
...
#TRENDLINES FOR EACH YEAR OF DATA: 

#2016 plot: [1] "y = 268.66 + -14.37 * x + 0.24 * x^2"
#2012 plot: [1] "y = 340.21 + -16.14 * x + 0.28 * x^2"
#2008 plot: [1] "y = 500.23 + -24.45 * x + 0.37 * x^2"
#2004 plot: [1] "y = 407.93 + -19.86 * x + 0.33 * x^2"
#2000 plot: [1] "y = 118.93 + -4.8 * x + 0.07 * x^2"
...
f_2000 <- function(x) { 118.93 - 4.8 * x + 0.07 * x^2 }
f_2004 <- function(x) { 407.93 - 19.86 * x + 0.33 * x^2 }
f_2008 <- function(x) { 500.23 - 24.45 * x + 0.37 * x^2 }
f_2012 <- function(x) { 340.21 - 16.14 * x + 0.28 * x^2 }
f_2016 <- function(x) { 268.66 - 14.37 * x + 0.24 * x^2 }
...
labels <- data.frame(
  ...
  x = rep(8, 5),  
  y = c(118.93, 407.93, 500.23, 340.21, 268.66)  
)
...
charttt <- ggplot(data_long, aes(x = x, y = y, color = factor(year))) + 
  ...
  labs(title = "Relationship Trend By Year",
       ...
       y = "Patents per Capita",
       color = "Year") +
  scale_color_manual(values = c("year_2000" = "blue", "year_2004" = "orange", "year_2008" = "green", "year_2012" = "red", "year_2016" = "purple")) +
  theme_minimal()

charttt


## histogram patent figures 
...
sum(data2000$patent, na.rm = TRUE)

#year 2000: 213,944 patents -- about 1/4 of total number 
#year 2004 : 1.002,361 patents 
#year 2008: 1.224,441 patents
#year 2012: 1.176,977 patents 
#year 2016: 693,304 patents 
